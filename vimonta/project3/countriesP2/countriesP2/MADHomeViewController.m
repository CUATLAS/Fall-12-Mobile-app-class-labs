...
@interface MADHomeViewController () {
    ...
    NSMutableDictionary *filteredCountries;
    NSArray *countryDetails;
}
...
- (BOOL)valuesInRange: (NSArray *)countryDetail {
    NSString *valueStr = (NSString *)[countryDetail objectAtIndex:[MADStoredData sharedInstance].currentSegment];
    float floatVal = [valueStr floatValue];
    
    if ((floatVal >= [MADStoredData sharedInstance].lowValue) && (floatVal <= [MADStoredData sharedInstance].highValue)) {
        return YES;
    }
    /*else if (floatVal <= [MADStoredData sharedInstance].highValue) {
        NSLog(@"floatVal: %.1f highVal: %.1f", floatVal, [MADStoredData sharedInstance].highValue);
        return YES;
    }*/
    return NO;
}
...
- (IBAction)showTheTable:(UIButton *)sender {
    ...
    if ([MADStoredData sharedInstance].currentSegment != -1) {
        filteredCountries = [[NSMutableDictionary alloc] init];
        countryDetails = [[NSArray alloc] init];
        
        for (NSString *key in countryData) {
            countryDetails = [countryData objectForKey:key];
            if ([self valuesInRange:countryDetails]) {
                [filteredCountries setObject:countryDetails forKey:key];
            }
        }
        masterViewController.filteredCountryData = filteredCountries;
        masterViewController.isFiltered = YES;
    }
    else {
        masterViewController.isFiltered = NO;
    }
